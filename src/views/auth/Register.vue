        ...
        try {
          appAxios.post('/company/v1/user', formData).then(async res => { 
            if(res.status === 201){
              ...
              loading.value = false
              toast({status: 'error', title: 'register error', content: res.error.response.data})    
            }
          })
        } catch(err) {
          toast({status: 'error', title: 'register error', content: 'Something was wrong in Register. Please try again later'})
        }
